import { libBySlug, libParams, DEFAULT_POOL_TAGS } from './libraries';
...
// The collection params a topic's books are drawn from: `tags` (+ `match`) for the
// new API, `lib` riding along for the pre-tags API (see lib/api.js callers).
export const topicPool = (topic) => {
  if (!topic.lib) return { tags: DEFAULT_POOL_TAGS };
  return { ...libParams(libBySlug(topic.lib)), lib: topic.lib };
};
