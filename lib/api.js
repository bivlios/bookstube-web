// Thin client for the Books Giant public API. Content reads below are GET + ISR-cached;
// server routes may also use the exported base URL for tightly scoped POST endpoints.
export const BOOKSTUBE_API_BASE =
...
async function apiGet(path, revalidate) {
  const res = await fetch(`${BOOKSTUBE_API_BASE}${path}`, { next: { revalidate } });
...
export const viewPingUrl = (bookId) => `${BOOKSTUBE_API_BASE}/view/${encodeURIComponent(bookId)}`;
