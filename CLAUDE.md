...
- The curated library list (pills/switcher) is hardcoded in `lib/libraries.js` — edit + push, no DB. Each entry's `tags` array defines its CONTENT (passed to the API as a `tags` param — no TagLibraries doc needed on the Books Giant side; the legacy `lib` id param still rides along for back-compat and legacy /taglib/<id> URLs). `home: true` marks the collection served at "/". `LIBRARY_ORDER` maps each site language to the collections it shows and their pill order; a collection absent from a language's list gets no pill/sitemap entry there and its URL is noindexed in that language.
